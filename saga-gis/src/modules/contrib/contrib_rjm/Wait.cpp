...
CWait::CWait(void)
{
	...
	Set_Description	(_TW("Wait for jobs on a remote cluster and download results."));
...
bool CWait::JobsDone(CSG_String joblist)
{
	bool alldone = false;
	CSG_File File;
	CSG_String jobdir;
...
bool CWait::ConfigExists()
{
	return SG_File_Exists(RJMConfigFilePath);
}

void CWait::ReadConfig()
{
