...
CCancel::CCancel(void)
{
	Set_Name(_TL("Cancel"));
	Set_Author		(SG_T("Sina Masoud-Ansari"));
	Set_Description	(_TW("Cancel scheduled jobs on a remote cluster"));
	...
	RJMBatchCancel = SG_File_Make_Path(RJMBinDir, CSG_String("rjm_batch_cancel"), CSG_String("exe"));
...
bool CCancel::On_Execute(void)
{
	...
	if (Process_Get_Okay(true))
	{
		...
		{
			Message_Dlg(CSG_String::Format(SG_T("Error while cancelling remote jobs, see Execution Log for details")));
			DisplayRJMLog();
			return false;
		}	
...
bool CCancel::GetParameterValues()
{
...
bool CCancel::ConfigExists()
{
	return SG_File_Exists(RJMConfigFilePath);
}

void CCancel::ReadConfig()
{
