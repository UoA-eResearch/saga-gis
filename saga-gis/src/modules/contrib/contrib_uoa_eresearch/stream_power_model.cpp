...
Cstream_power_model::Cstream_power_model(void)
{
	...
	*/
	...
	Parameters.Add_Value(
		NULL	, "LATTICE_SIZE_X"	, _TL("X Dimension"),
		_TL("X dimension for generated landscape"),
		PARAMETER_TYPE_Int, 250);

	Parameters.Add_Value(
		NULL	, "LATTICE_SIZE_Y"	, _TL("Y Dimension"),
		_TL("Y dimension for generated landscape"),
		PARAMETER_TYPE_Int, 250);
...
bool Cstream_power_model::On_Execute(void)
{
	...
	lattice_size_x = Parameters("LATTICE_SIZE_X")->asInt();
	lattice_size_y = Parameters("LATTICE_SIZE_Y")->asInt();
	//InputGrid = NULL;

	OutputGrid = SG_Create_Grid(SG_DATATYPE_Float, lattice_size_x, lattice_size_y, 1.0, 0.0, 0.0);
	OutputGrid->Set_Name(CSG_String::Format(SG_T("%s"), _TL("Random Elevation")));
	OutputGrid->Assign_NoData();
	Parameters("OUTPUT")->Set_Value(OutputGrid);
	...
	if (!InputGrid)
	{
		 ...
		 Parameters("LATTICE_SIZE_X")->Set_Value(lattice_size_x);
		 Parameters("LATTICE_SIZE_Y")->Set_Value(lattice_size_y);
		 OutputGrid = SG_Create_Grid(InputGrid);
		 OutputGrid->Assign_NoData();
	}
	*/
...
void Cstream_power_model::testWrite(void)
{
...
int **Cstream_power_model::imatrix(long nrl, long nrh, long ncl, long nch)
...
void Cstream_power_model::indexx(int n, float *arr, int *indx)
{
...
void Cstream_power_model::fillinpitsandflats(int i, int j)
{    float min;
...
void Cstream_power_model:: mfdflowroute(int i, int j)
{    float tot;
...
void Cstream_power_model::calculatealongchannelslope(int i, int j)
{    float down;
...
void Cstream_power_model::avalanche(int i, int j)
{
...
void Cstream_power_model::free_ivector(int *v, long nl, long nh)
/* free an int vector allocated with ivector() */
{
        free((FREE_ARG) (v+nl-NR_END));
}

void Cstream_power_model::free_vector(float *v, long nl, long nh)
/* free an int vector allocated with ivector() */
{
        free((FREE_ARG) (v+nl-NR_END));
}

void Cstream_power_model::tridag(float a[], float b[], float c[], float r[], float u[], unsigned long n)
{
	unsigned long j;
	float bet,*gam;
...
void Cstream_power_model::hillslopediffusioninit()
{    int i,j,count, D;
     float term1;
...
float **Cstream_power_model::matrix(long nrl, long nrh, long ncl, long nch)
...
float Cstream_power_model::ran3(int *idum)
{
...
float Cstream_power_model::gasdev(int *idum)
{
	static int iset=0;
	static float gset;
	float fac,r,v1,v2;
...
float *Cstream_power_model::vector(long nl, long nh)
...
int *Cstream_power_model::ivector(long nl, long nh)
...
void Cstream_power_model::setupgridneighbors()
{   
	int i,j;
...
void Cstream_power_model::streampower()
{   
	 FILE *fp1;
