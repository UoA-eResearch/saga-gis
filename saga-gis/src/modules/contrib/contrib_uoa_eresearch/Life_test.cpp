...
bool CLife::On_Execute(void)
{
	...
	//-----------------------------------------------------
	pLife	= SG_Create_Grid(SG_DATATYPE_Float, Parameters("NX")->asInt(), Parameters("NY")->asInt());
	...
	*/
	...
	//-----------------------------------------------------
	for(i=1; Process_Get_Okay(true) ; i++)
	{
		RandomUpdate();
	...
	DoNothing();
	//RandomUpdate(); // this is the problem, data is made dirty after check ok.
	//-----------------------------------------------------
	//delete(pCount);
...
void CLife::DoNothing()
{
	pLife->Set_Value(0, 0, 3.2f);
}

bool CLife::RandomUpdate()
{

	for(int y=0; y<pLife->Get_NY(); y++)
	{
		for(int x=0; x<pLife->Get_NX(); x++)
		{
			float value = rand() % nColors;
			pLife->Set_Value(x, y, value);
		}
