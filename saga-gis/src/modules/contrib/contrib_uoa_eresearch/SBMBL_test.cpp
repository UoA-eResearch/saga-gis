...
void CSBMBL::main()
{
	...
	while ( (RunTimeClock() < maxRunTime*FORCING_DURATION) && Process_Get_Okay(true) )  
    {
		...
		*/
		...
		for(p=1;p<=AdjustTime && Process_Get_Okay(true);p++) {
			DoIterationDummy();    /*Gives sed trans across boundaries time*/
			/*to adjust to new direction*/
		} /* // for */

		while (ForcingClock() < FORCING_DURATION && Process_Get_Okay(true)) 
		{	  
			DoIteration();  
...
void CSBMBL::UpdateGrid()
{
	...
	for(int y=0; y < Ymax; y++)
	{
		for(int x=0; x < Xmax; x++)
		{
			...
			pGridHeight->Set_Value(x, y, height * 255);
...
CSBMBL::CSBMBL(void)
{
	...
	Parameters.Add_Value(NULL, "maxRunTime",
...
bool CSBMBL::On_Execute(void)
{
	...
	// Height
	pGridHeight = SG_Create_Grid(SG_DATATYPE_Byte, (int)Xmax, (int)Ymax, CELL_WIDTH);
